	...
	d.fn.extend({
		renderCalendar: function (t) {
			...
			var e = d(C('table'))
				...
				.addClass('jCalendar')
				.append(t.showHeader != d.dpConst.SHOW_HEADER_NONE ? d(C('thead')).append(o) : C('thead'));
			var f = d(C('tbody'));
			...
			while (n++ < q) {
				...
				for (var x = 0; x < 7; x++) {
					...
					var y = d(C('td'))
						.text(m.getDate() + '')
						.addClass(
							(j ? 'current-month ' : 'other-month ') + (m.isWeekend() ? 'weekend ' : 'weekday ') + (j && m.getTime() == z.getTime() ? 'today ' : '')
						)
						.data('datePickerDate', m.asString())
						.hover(A(k), g);
					u.append(y);
					if (t.renderCallback) {
		...
		datePicker: function (e) {
			...
			return this.each(function () {
				...
				f.init(e);
				if (!i && e.createButton) {
					f.button = d('<a href="#" class="dp-choose-date" title="' + d.dpText.TEXT_CHOOSE_DATE + '">' + d.dpText.TEXT_CHOOSE_DATE + '</a>').bind(
						'click',
						function () {
							g.dpDisplay(this);
							this.blur();
	...
	d.extend(a.prototype, {
		...
		display: function (e) {
				...
				d('<div></div>')
					...
					.append(
						...
						d('<div class="dp-nav-prev"></div>').append(
							d('<a id="dp-nav-prev-year" href="#" title="' + d.dpText.TEXT_PREV_YEAR + '">&laquo;</a>').bind('click', function () {
						...
						d('<div class="dp-nav-next"></div>').append(
							d('<a id="dp-nav-next-year" href="#" title="' + d.dpText.TEXT_NEXT_YEAR + '">&raquo;</a>').bind('click', function () {
								return m._displayNewMonth.call(m, this, 0, 1);
							}),
			...
			if (this.displayClose) {
				f.append(
					d('<a href="#" id="dp-close">' + d.dpText.TEXT_CLOSE + '</a>').bind('click', function () {
						m._closeCalendar();
						return false;
					})
				);
